...
const createAppointment = async (req, res) => {
  ...
  try {
    ...
    res.status(201).json({ msg: 'Tu reservación se realizó correctamente' })
  } catch (error) {
    console.log(error)
  }
}
...
const getAppointmentById = async (req, res) => {
  ...
  if (!appointment) {
    return handleNotFoundError(res, 'La reservación no existe')
  }
  if (req.user._id.toString() !== appointment.user.toString()) {
    const error = new Error('No tienes permiso para ver esta reservación')
...
const updateAppointment = async (req, res) => {
  ...
  try {
    ...
    res.json({ msg: 'La reservación se actualizó correctamente' })
  } catch (error) {
    console.log(error)
  }
}
